var MainFunction = {
    ...
    saveStatusUser: function () {
        ...
        $('.table-checkbox input:checked').each(function (indx, element) {
            /*
             if(indx == 0)
             {
                 $('.table-checkbox input').each(function(i, el) {
        ...
        /*
        if($('.table-checkbox input').prop('checked'))
        {
            var statusId = $('.selectric-wrapper:eq(0)').find('select').val();
        }
        else
        {
           var statusId = $('.selectric-wrapper:eq(' + index + ')').find('select').val(); 
        }
        */

        var statusId = $('.changedStatus').parent().parent().find('select').val();
        if (arr && statusId) {
            $.ajax({
                type: 'post',
    ...
    checkStatus: function (e) {
        ...
        $(document).find('.cash-coins').val(300);
    ...
    startTimer: function (duration, timer, callback, type) {
    ...
    getBonuslotto: function (winner, countElement = 8, minValue = 2, maxValue = 5, width = 80, seconds = 0, callback = false, e, element = 'slick-track') {
        ...
        function go() {
            ...
            var increased = 0;
            if ($(window).width() <= 1360 && $(window).width() > 1280) {
                increased = (1360 - $(window).width()) / 1.4;
            } else if ($(window).width() <= 1280 && $(window).width() > 500) {
                increased = 140;
            } else if ($(window).width() <= 500) {
                increased = (500 - $(window).width()) / 1.6333;
            }
            carusel.animate({marginLeft: -8888 - randomInteger(0, 70) - increased}, 10000, 'easeOutCubic',
                function () {
    ...
    lotto: function (winner, countElement = 8, minValue = 2, maxValue = 500, width = 80, seconds = 0, callback = false, e, winners, element = 'slick-track') {
        ...
        function go() {
            ...
            if ($(window).width() <= 1360 && $(window).width() > 1280) {
                ...
                increased = 140 + (500 - $(window).width()) / 2;
            }
            carusel.animate({marginLeft: -8140 - randomInteger(0, 70) - increased}, 10000, 'easeOutCubic',
                function () {
        ...
        if (userId && applicationId) {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $('.wrap-modal-user').find('select').selectric({
...
var Payment = {
    ...
    withdrawalOfFunds: function (element) {
        ...
        var paymentType = $('#outputModal').find('input[name="outputType"]:checked').val();
...
let switchHomeLayout = function (layoutId) {

    var typeGameId = layoutId;
    ...
    if (typeGameId && currentGameId && currentGameId != typeGameId) {
        $.ajax({
            ...
            success: function (result) {
                ...
                // src = $('script[src$="/js/scripts.min.js"]').attr("src");
                // $('script[src$="' + src + '"]').remove();
                // $('<script/>').attr('src', src).appendTo('body');
                // src = $('script[src$="/js/script.js"]').attr("src");
                // $('script[src$="' + src + '"]').remove();
                // $('<script/>').attr('src', src).appendTo('body');
                // applyProjectEvents()
...
let applyProjectEvents = function () {
    ...
    $('.modal__close-link').on('click', function () {
    ...
    $('.modal__bottom .paginations__item a').on('click', function (e) {
        var el = $(this);
        e.preventDefault();
        var page = el.attr('data-page');
